...
export function usePokemonTypes(speciesId?: number, form: number = 0) {
	const [types, setTypes] = useState<{ type1?: string; type2?: string; colors: { type1?: string; type2?: string } }>({
		colors: {}
	})
	...
	useEffect(() => {
		...
		const fetchPokemonTypes = async () => {
			...
			try {
				...
				console.log(`Loaded types for Pokemon ${speciesId} (form ${form}):`, { type1, type2 })
			} catch (err) {
